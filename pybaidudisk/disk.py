...
class NetDisk(object):
    ...
    def api_request(self, url, method="GET", extra_data=dict(), retry_limit=2, encoding=None, **params):
        ...
        return data
    def doCookie(self):
        string = "HttpOnly_.baidu.com"
        cookieFile = open(self.cookie_file,"r")
        lines = cookieFile.readlines()
        newlines = ''
        for line in lines:
            if string in line:
                line = line.replace("#","#")
                #line = "#HttpOnly_.baidu.com\tTRUE\t/\tFALSE\t1695008297\tBDUSS\tmg5bTJCUVRWMHJYbH5jfnh-UC1CWmFFMXZMT1J1eWp3QjJ2WkJSYVN-NHBQN3hWQVFBQUFBJCQAAAAAAAAAAAEAAAA0Oo8Cc2FnaWxsZXIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAACmylFUpspRVN"
                #line = line[1:]
            newlines += line
        cookieFile.close()
        cookieFile = open(self.cookie_file,"w")
        cookieFile.write(newlines)
        cookieFile.close()
    def _bduss(self):
